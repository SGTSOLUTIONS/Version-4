...
<div class="row g-3 mb-4">
    <div class="col-12">
        <div class="ds-card">
            ...
            <div class="ds-card-body" style="padding:0; position:relative;">
                <div id="commissioner-map" style="width:100%; height:500px; background:#f0f5f9; border-radius:0 0 12px 12px;"></div>

                <!-- Loading indicator -->
                <div id="map-loading" style="position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); z-index:5; background:rgba(255,255,255,0.9); padding:20px 30px; border-radius:12px; box-shadow:0 4px 20px rgba(0,0,0,0.1); display:flex; align-items:center; gap:12px;">
                    <div class="spinner-border text-success" role="status" style="width:24px; height:24px;"></div>
                    <span style="color:#0a2e1a; font-weight:500;">Loading map...</span>
                </div>
            </div>
        </div>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const wardBoundaries = mapData ? mapData.boundaries : null;
        const wardCenters = mapData ? mapData.centers : [];

        console.log('Map Data:', mapData);
        console.log('Boundaries:', wardBoundaries);
        console.log('Features count:', wardBoundaries ? wardBoundaries.features.length : 0);

        if (!wardBoundaries || !wardBoundaries.features || wardBoundaries.features.length === 0) {
            // No boundaries found - show message
        ...
        // Convert GeoJSON features to OpenLayers features with validation
        let featureCount = 0;
            ...
            try {
                ...
                if (feature.geometry.type === 'Polygon') {
                    ...
                    if (coordinates.length < 4) {
                        console.warn('Invalid polygon (less than 4 points):', coordinates);
                        return;
                    }
        ...
        console.log('Added ' + featureCount + ' features to map');

        if (featureCount === 0) {
            loadingElement.style.display = 'none';
            mapContainer.innerHTML = `
                <div style="display:flex; align-items:center; justify-content:center; height:100%; flex-direction:column; color:#9ca3af; background:#f9fafb; border-radius:0 0 12px 12px;">
                    <i class="bi bi-exclamation-triangle" style="font-size:3rem; opacity:0.3; margin-bottom:1rem; color:#f59e0b;"></i>
                    <p style="font-size:0.9rem; font-weight:500;">No valid boundaries found</p>
                    <p style="font-size:0.75rem;">The geometry data could not be parsed</p>
                </div>
            `;
            return;
        }
        ...
        tooltipElement.style.cssText = 'display:none; position:absolute; background:rgba(0,0,0,0.85); color:white; padding:8px 14px; border-radius:8px; font-size:0.75rem; pointer-events:none; z-index:20; max-width:250px; backdrop-filter:blur(4px);';
        ...
        controlsDiv.style.cssText = 'position:absolute; top:20px; right:20px; display:flex; gap:6px; flex-direction:column; z-index:10;';
        controlsDiv.innerHTML = `
            <button onclick="window.zoomIn()" class="map-control-btn" style="width:36px; height:36px; border-radius:8px; border:1px solid #e5e7eb; background:white; color:#1f2937; font-size:18px; font-weight:600; cursor:pointer; transition:all 0.15s; display:flex; align-items:center; justify-content:center; box-shadow:0 2px 8px rgba(0,0,0,0.08);">
                <i class="bi bi-plus-lg"></i>
            </button>
            <button onclick="window.zoomOut()" class="map-control-btn" style="width:36px; height:36px; border-radius:8px; border:1px solid #e5e7eb; background:white; color:#1f2937; font-size:18px; font-weight:600; cursor:pointer; transition:all 0.15s; display:flex; align-items:center; justify-content:center; box-shadow:0 2px 8px rgba(0,0,0,0.08);">
        ...
        legendDiv.style.cssText = 'position:absolute; bottom:20px; right:20px; background:white; padding:12px 16px; border-radius:10px; box-shadow:0 4px 20px rgba(0,0,0,0.15); font-size:0.75rem; z-index:10; max-height:200px; overflow-y:auto; min-width:120px;';

        let legendHtml = '<strong style="display:block; margin-bottom:6px; color:#0a2e1a;">Ward Legend</strong>';
        const wardColors = {};
        ...
        Object.values(wardColors).slice(0, 15).forEach(function(item) {
            legendHtml += `
                <div style="display:flex; align-items:center; gap:8px; padding:2px 0;">
                    <span style="width:14px; height:14px; border-radius:4px; background:${item.color}; display:inline-block;"></span>
        ...
        console.log('🗺️ Commissioner Map initialized with ' + featureCount + ' ward boundaries');
    });
</script>
@endpush

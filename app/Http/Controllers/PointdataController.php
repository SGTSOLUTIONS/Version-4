...
class PointdataController extends Controller
{
    ...
    public function pointDataStore(Request $request)
    {
        ...
        try {
            ...
            // ============================================================
            // GET ZONE
            // Request zone has priority.
            // If request zone is empty, get zone from polygon_data.
            // ============================================================
            $finalZone = $request->filled('zone')
                ? $request->zone
                : ($buildingdata->zone ?? null);

            if (empty($finalZone)) {
                return response()->json([
                    'success' => false,
                    'message' => 'Zone value not found in request or polygon data.'
                ], 422);
            }
            // Validate floor
                            ...
                            if (!empty($existing->gisid) && $existing->gisid != $request->point_gisid) {
                                ...
                                DB::table($professionalTaxTableName)
                                    ...
                                    ->update([
                                        ...
                                        'gisid' => $request->point_gisid,
                                        'assessment' => $request->assessment,
                                        'owner_name' => !empty($professional['shop_owner']) && $professional['shop_owner'] !== '0'
                                            ? $professional['shop_owner']

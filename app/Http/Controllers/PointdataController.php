...
class PointdataController extends Controller
{
    ...
    // }
      ...
      /**
 * Save/update the QC fields for a point_data record
 */
public function qcUpdate(Request $request, $id)
{
    $validator = Validator::make($request->all(), [
        'qcusage'    => 'nullable|in:Residential,Commercial,Mixed,Vacant',
        'qcsqfeet'   => 'nullable|numeric|min:0',
    ...
    try {
        ...
        if (!Schema::hasTable($pointDataTable)) {
            return response()->json(['success' => false, 'message' => "Table {$pointDataTable} not found"], 404);
        }
        ...
        $updated = DB::table($pointDataTable)->where('id', $id)->first();
...
/**
 * Get all point data with water_tax, ugd_tax, professional_tax for a GIS ID
 */
public function getByGisId($gisid)
{
    try {
        $user = User::find(Auth::id());
        if (!$user) {
        ...
        foreach ($pointData as $pd) {
            // Get water tax by assessment
            $waterTax = null;
            if ($pd->assessment) {
                $waterTax = DB::table($waterTaxTable)
                    ->where('assessment', $pd->assessment)
                    ->first();
            }

            // Get UGD tax by gisid
            $ugdTax = DB::table($ugdTaxTable)
                ->where('gisid', $pd->point_gisid)
                ->first();

            // Get Professional tax by gisid AND assessment
            $professionalTax = DB::table($professionalTaxTable)
                ->where('gisid', $pd->point_gisid)
                ->where('assessment', $pd->assessment)
                ->first();
...
/**
 * Filter point data with phone_number search
 */
public function pointDataFilter(Request $request)
{
    $user = User::find(Auth::id());

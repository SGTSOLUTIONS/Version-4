...
class CommissionerController extends Controller
{
    ...
    /**
     * Get building variation data (helper method)
     */
    private function getBuildingVariationData($wardId, $gisid)
    {
        ...
        $buildingUsage = $polygonData->building_usage ?? null;
        $assessmentUsage = !empty($points) ? ($points[0]['qcusage'] ?? $points[0]['bill_usage'] ?? null) : null;

        $usageStatus = 'NO_DATA';
        $usageStatusLabel = 'No Data';
        $usageBadgeClass = 'no-data';

        if ($buildingUsage && $assessmentUsage) {
            if (strtoupper(trim($buildingUsage)) == strtoupper(trim($assessmentUsage))) {
                $usageStatus = 'MATCH';
                $usageStatusLabel = 'Match';
                $usageBadgeClass = 'match';
            } else {
                $usageStatus = 'VARIATION';
                $usageStatusLabel = 'Variation';
                $usageBadgeClass = 'variation';
            }
        } elseif ($buildingUsage && !$assessmentUsage) {
            $usageStatus = 'BUILDING_ONLY';
            $usageStatusLabel = 'Building Only';
            $usageBadgeClass = 'building-only';
        } elseif (!$buildingUsage && $assessmentUsage) {
            $usageStatus = 'ASSESSMENT_ONLY';
            $usageStatusLabel = 'Assessment Only';
            $usageBadgeClass = 'assessment-only';
        }
        ...
        return [
            ...
            'assessment' => [
                'area' => $assessmentArea,
                'usage' => $assessmentUsage ?? 'N/A',
                'count' => count($points),
                'all_usages' => array_unique(array_column($points, 'qcusage')),
                'has_multiple' => count($points) > 1,
                'details' => [
                    'points' => $points,
                    'assessment_type_status' => !empty($points) ? ($points[0]['assessment_type'] ?? 'N/A') : 'N/A',
                ]
            ],
            'area_comparison' => [
                'building_area' => $buildingArea,
            ...
            'usage_comparison' => [
                ...
                'all_assessment_usages' => array_unique(array_column($points, 'qcusage')),
                'has_mismatch' => $usageStatus === 'VARIATION',

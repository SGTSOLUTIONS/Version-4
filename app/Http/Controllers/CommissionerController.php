...
class CommissionerController extends Controller
{
    ...
    public function showMap($id)
    {
        try {
            ...
            // Get building data with usage colors
            ...
            // Calculate total sq feet for polygons
            foreach ($polygons as $polygon) {
            ...
            // FIXED: Changed 'excecutive' to 'executive'
            ...
            abort(404, 'Map data not found. Error: ' . $e->getMessage());
        }
    }
    ...
    private function getAccessibleWardIds()
    {
        try {
            ...
            return Ward::whereIn('zone_id', $zoneIds)->pluck('id')->toArray();
        } catch (\Exception $e) {
    ...
    public function getPointDetails(Request $request)
    {
        try {
            ...
            Log::error('Error in getPointDetails: ' . $e->getMessage());
            return response()->json([
                'status' => false,
                'message' => 'Error loading point details: ' . $e->getMessage()
            ], 500);
        }
    }
    ...
    public function qcUpdate(Request $request, $id)
    {
        try {
            ...
            Log::error('Error in qcUpdate: ' . $e->getMessage());
            return response()->json([
                'success' => false,
                'message' => 'Error updating QC data: ' . $e->getMessage()
            ], 500);
        }
    }
